  ...
  return (
    <div className={compact ? 'grid grid-cols-2 gap-1.5' : 'grid grid-cols-4 gap-2'}>
      {statCards.map((stat) => (
        <div
          key={stat.label}
          className={`glass-card bg-gradient-to-br from-primary/20 via-card/90 to-energy/15 rounded-lg ${compact ? 'p-2' : 'p-3'} shadow-md ${onClick ? 'cursor-pointer hover:brightness-95 dark:hover:brightness-110 transition-all' : ''}`}
          onClick={onClick}
        >
