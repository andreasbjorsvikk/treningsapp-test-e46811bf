...
const PeaksList = ({ peaks, checkins, onSelectPeak }: PeaksListProps) => {
          ...
          {filtered.map(peak => {
            ...
            return (
              <button
                ...
                className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border/50 hover:border-primary/30 transition-colors text-left"
              >
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${
                  isTaken ? 'bg-success/15 text-success' : 'bg-muted text-muted-foreground'
                }`}>
