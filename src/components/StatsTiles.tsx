...
const StatsTiles = ({ sessions }: StatsTilesProps) => {
  ...
  return (
    <div className="grid grid-cols-4 gap-2">
      {tiles.map((tile) => (
        <div key={tile.label} className="glass-card rounded-lg p-3 flex flex-col items-center text-center shadow-md" style={{ background: 'linear-gradient(to bottom right, hsl(var(--primary) / 0.16), hsl(var(--primary) / 0.07), hsl(var(--primary) / 0.13))' }}>
